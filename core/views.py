...
def order_status(request):
    orders = Order.objects.filter(user=request.user).order_by('-ordered_date')  # Fetch all orders
...
def add_to_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        order.items.add(order_item)
        messages.info(request, "New order was created and item was added.")

    print(f"DEBUG: Order exists: {order}")  # Debugging log
    return redirect("core:order-summary")
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                order = Order.objects.get(user=self.request.user, ordered=False)
                coupon = get_coupon(self.request, code)  # Get the coupon
